...
[![Build](https://github.com/stellarshenson/forensic-simulations/actions/workflows/build.yml/badge.svg)](https://github.com/stellarshenson/forensic-simulations/actions/workflows/build.yml)
...
> [!IMPORTANT]
> The event **did not take place**. There has never been any violence, or anything even approaching violence, there. The father is using science, physics, simulation, geometry and statistics to prove to the court what an elevator-door throw of the kind described would necessarily leave behind - a posterior rib fracture, a 120 dB metallic clang, a body impact loud enough to clip the phone microphone that recorded the whole visit - and that none of that fingerprint appears anywhere in the documented evidence.
>
...
At the gentlest reading the accusation allows, the throw is physically feasible inside three seconds - the corridor motion is mild, the two 180-degree rotations are what strain the time budget. Feasible is not the same as consequence-free. The deformable-body impact model puts the realistic peak force at **5.5 to 6.4 kN** (the rigid-wall kinematic bound is **16 to 22 kN**), enough to make a posterior rib fracture one of eight highly probable injuries, and the collision radiates a **120 dB** metallic door clang and a **100 dB** body thump. The medical examination afterwards documented one bruise on the right shoulder and no rib fracture. Two independent recordings of the visit contain no clang, no thump, no clipping spike. The third-party witness, a metre and a half away, reports no loud noise of any kind.

You can decide for yourself what that means. The reconstruction cheats on the prosecution's behalf throughout - the accusation gets the friendliest possible reading - and the mechanical and acoustic fingerprint it predicts is still absent from every part of the documented record.

## In my defence

Let me say it before the court does: this is not a normal response to a legal problem. A well-adjusted adult, wrongly accused, hires a lawyer, files a measured reply, and lets a professional walk him through his feelings. I voxelised a 1988 elevator door. I am now, entirely against my will, a domain expert in the flexural modes of communist-era lift hardware - a qualification with, I have checked, no resale value of any kind. There are five notebooks in here, a tetrahedral mesh of a human torso, and an aeroacoustic estimate of the air squeezed out from behind a falling body, because somewhere along the way "prove you didn't do it" quietly turned into "model the radiation efficiency of the thing you didn't do it with." I pinned the eigensolver's random seed so the worst three seconds of my family's year would come out bit-for-bit identical on every run. I called that scientific rigour. It was so the numbers would hold still while I stared at them - which is cheaper than therapy and, unlike therapy, has 96% test coverage.
...
Every number in the analysis is reproducible by running the notebooks against the parameters in `simulation_config.yaml`; ruff-clean Python, pytest-covered, managed by `uv`.
...
- Peak SPL at 1 m is about 100 dB flat, about 80 dBA A-weighted - the surface texture is what lifts it into the band a meter and the ear register; six figures and a WAV generated inline, the model exercised by 31 test guards in [`tests/test_bodyfem.py`](tests/test_bodyfem.py)
...
- scikit-fem assembles the box's 3D linear-elastic stiffness and mass; an eigensolve gives the leaf's flexural modes, from about 146 Hz upward - the welded box is stiff, which is what makes the door clang rather than boom
...
- Peak SPL at 1 m is about 120 dB flat, ~107 dBA - a bright metallic clang in the door's mode band, distinct from and considerably louder than the body thump; four figures and a WAV generated inline, the model exercised by 14 test guards in [`tests/test_doorfem.py`](tests/test_doorfem.py)
...
- The augmented recording is written to `reports/figures/augmented_event_recording.m4a`; the model is exercised by 14 test guards in [`tests/test_audiomix.py`](tests/test_audiomix.py)
...
notebooks/                            01 kinematics, 02 impact, 03 body thump, 04 door clang, 05 event audio
src/henryk_simulations/corridor/      choreography, impact, injuries, bodyfem, doorfem, reverb, audiomix, sim
reports/figures/                      generated figures, the MP4, the impact-sound WAVs and the augmented recording
...
Crash reconstruction asks whether a specific impact happened. This test asks a different question: what an impact of the kind described would necessarily leave behind, and whether the documented record shows any of it. Four years of contested family-court litigation, substantially clarified by twenty minutes of biomechanics.
